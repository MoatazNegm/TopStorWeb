            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left">
                    <thead>
                        <tr className="bg-gray-50/50 text-gray-400 text-[10px] font-black uppercase tracking-widest">
                            <th className="px-6 py-4">User / Volume</th>
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {volumes.map((vol) => (
                            <tr key={vol.name} className="hover:bg-indigo-50/30 transition-colors group/row">
                                ...
                                <td className="px-6 py-5">
                                    {editingId === vol.name ? (
                                        <input
                                            ...
                                            className="w-32 px-2 py-1 text-sm border border-indigo-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
                                        />
                                    ) : (
                                ...
                                <td className="px-6 py-5">
                                    {editingId === vol.name ? (
                                        <input
                                            ...
                                            className="w-16 px-2 py-1 text-sm border border-indigo-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
                                        />
                                    ) : (
                                ...
                                <td className="px-6 py-5 text-right">
                                    <div className="flex justify-end gap-2 opacity-0 group-hover/row:opacity-100 transition-opacity">
                                        {editingId === vol.name ? (
                                            ...
                                            <>
                                                <button onClick={() => handleEdit(vol)} className="p-2 text-indigo-500 hover:bg-indigo-50 rounded-lg transition-colors" title="Edit">
                ...
                {volumes.length === 0 && (
                    <div className="py-20 text-center">
                        ...
                        <p className="text-gray-400 font-medium tracking-tight">No Home folders found</p>
                    </div>
                )}
            </div>
        </div>
