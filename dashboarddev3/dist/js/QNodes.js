    ...
    if (JSON.stringify(allhosts[status]) != JSON.stringify(newhosts[status])) {
      ...
      $.each(hosts, function (e, host) {
        ...
        $("#hosts" + status).append(
          ...
          + '   <i class="fas fa-server fa-3x mb-2 ' + (imgstatus === 'On' ? 'text-success' : 'text-danger') + ' ' + status + 'SelectedFreewhite" style="text-shadow: 0 4px 6px rgba(0,0,0,0.1);"></i>'
...
function updaterunninghosts(status) {
  if (status == 'ready') {
    if (selectedhost[status] == "-1") {
      ...
      // Populate and select ports
      var ports = hostdata['ports'] || [];
      var portSelects = ['#nmports', '#cmports', '#dports'];
      portSelects.forEach(function (selector) {
        var $select = $(selector);
        $select.empty();
        ports.forEach(function (port) {
          $select.append(new Option(port, port));
        });

        // Determine selected values
        var key = selector.substring(1); // remove #
        var selected = hostdata[key];
      ...
      // Populate DNS
      $("#DNSname").val(hostdata['dnsname'] || '');
      $("#DNSsearch").val(hostdata['dnssearch'] || '');
...
$("#readysubmit").click(function (ev) {
  ...
  // Check Ports
  ['nmports', 'cmports', 'dports'].forEach(function (key) {
    var currentVal = $("#" + key).val() || [];
    var configVal = hostconfig[key];
  ...
  // Check DNS
  if ($("#DNSname").val() != hostconfig['dnsname']) {
    hostsubmit['dnsname'] = $("#DNSname").val();
    tochange = 1;
  }
  if ($("#DNSsearch").val() != hostconfig['dnssearch']) {
