                    ...
                    <div id="requesttable" class="row table-responsive">
                        <table class="col-12 table  dr-table-show">
                            ...
                            <tr id="Attachmirrored">
                                ...
                                <td class="text-center">add mirroed disk</td>
	...
	function setaction()	{
		...
		for (k in disks) {
			if(k > 0 ) {
		   	...
		   	if(disks[k].pool!="free"){ runningpool=disks[k].pool;runningroup=runningroup+disks[k].grouptype.substr(disks[k].grouptype.length-1)}
		...
		if(possiblenotstripe > 0 && foundselected==1)  {
			...
			if(possiblenotstripe==1 && runningpool!=0) {
				...
				if(dd[possiblenotstripe].grouptype=="free"){
					...
					if(runningroup.indexOf('rr') > -1 && minspace > 0) {
						...
						if(parseInt(dd["1"].size) >= minspace) { $("#Addspare").show();}
			...
			if(possiblenotstripe>1 && runningpool!=0) {
				...
				if(runningroup.indexOf('e') < 0 && possiblenotstripe==2) {
					maxspace=parseInt(dd["1"].size);
				...
				if(runningroup.indexOf('e') < 0 && possiblenotstripe==3) {
				...
				if(runningroup.indexOf('e') < 0 && possiblenotstripe==4) {
					poolmsg="Pool p1 can be expanded by adding a single/Dual redunancy group of disks using disk "
					minspace=parseInt(dd["1"].size); maxspace=0;
					for(ddk in dd){
